...
$$ATT(g,t) = \E\left[ \left( \frac{\indicator{G=g}}{p_g} - \frac{p_g(\mathbf{X},Z)U}{(1-p_g(\mathbf{X},Z))p_g}\right)\Big(Y_{t} - Y_{g-1} - m_{gt}^0(\mathbf{X},Z)\Big) \right]$$
...
The most common practice is to just completely drop these covariates from the analysis, but there are alternatives
...
\begin{align*}
  ATT = \E\left[ \frac{D}{p} \Big(\Delta Y - m_0(X)\Big) \right] - \E\left[ \frac{p(X)(1-D)}{(1-p(X))p} \Big(\Delta Y - m_0(X)\Big)\right]
\end{align*}
