...
* In particular, could assume that $Y_{it} = Y_{it}(0)$ for $t < G_i - a$, then the new base period is $g-a$ for group $g$, and
    $$ATT(g,t) = \E[Y_t - Y_{g-a} | G=g] - \E[Y_t - Y_{g-a} | G=0]$$
...
* If you have aggregate data (e.g., county-level data), you could consider using a measure of the size of the unit (e.g., a county's population), then
    * If your outcome is some kind of aggregate variable (e.g., total employment or log(total emploment)), then you should consider weighting by county size
    * If your outcome is a rate / normalized per capita measure, then (tentatively) the case for weighting by county size is weaker
